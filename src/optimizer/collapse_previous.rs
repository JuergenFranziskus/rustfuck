...
pub fn collapse_previous(program: &mut InstructionNode) {
...
fn collapse_node_list(nodes: &mut Vec<InstructionNode>) {
    ...
    for mut node in nodes.split_off(0).into_iter() {
        if let NodeType::Previous(amount) = node.node_type {
            ...
            if let Some(incr) = current_incr.take() {
                new_nodes.push(InstructionNode {
                    node_type: NodeType::Previous(incr),
                    line: current_line,
                    char: current_char,
                });
            }

            collapse_previous(&mut node);
